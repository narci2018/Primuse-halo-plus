...
export function qqAudioCacheKey(mid) {
  return `qq:${mid}`;
}
...
export async function compatibleQQCacheGet(cacheKey, mediaMid, env) {
...
export async function searchQQ(keyword, limit, env, waitUntil) {
  const sources = [
...
export async function searchNetease(keyword, limit, env) {
...
export async function qqOfficialLyric(mid, signal = null) {
...
export async function qqAudioCandidates(mid, keyword, mediaMid, signal = null, { skipProxy = false, proxyDetail = null, proxyAttempted = false, env = null } = {}) {
  const candidates = [];
...
export async function selectVerifiedAudio(cacheKey, candidates, durationSeconds, identity = {}, env, { maxCount = 2, signal = null, timeoutMs = SELECT_AUDIO_TIMEOUT_MS, skipCache = false } = {}) {
  if (!skipCache) {
    const cached = await cacheGet(cacheKey, env);
